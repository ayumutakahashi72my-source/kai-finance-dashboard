  ...
  return (
    <article className="space-y-6 text-sm leading-relaxed" style={{ color: '#cbd5e1' }}>
      <div className="space-y-1">
        ...
        <h1 className="text-xl font-semibold" style={{ color: '#f1f5f9' }}>プライバシーポリシー</h1>
      ...
      <section className="space-y-3">
        <h2 className="font-semibold" style={{ color: '#f1f5f9' }}>1. 取得する情報およびその取得方法</h2>
        <p>本サービスは、以下の方法により、それぞれの情報を取得・収集します。</p>
        <ul className="space-y-3 pl-4" style={{ borderLeft: '2px solid #334155' }}>
          <li className="pl-3">
            <span className="font-medium" style={{ color: '#94a3b8' }}>認証・プロファイル情報：</span>
            <span> Google OAuth認証を介して取得する、ユーザーの氏名（表示名）、メールアドレス、プロフィール画像URL、および一意のアカウント識別子（UID）。</span>
          </li>
          <li className="pl-3">
            <span className="font-medium" style={{ color: '#94a3b8' }}>ユーザー登録データ：</span>
            <span> 取引履歴（日付、金額、店舗名・品名、カテゴリ等）、CSVファイルのインポートにより取得される取引データ、予算設定、財務目標、およびAIチャットへの入力テキスト（プロンプト）。</span>
          </li>
          <li className="pl-3">
            <span className="font-medium" style={{ color: '#94a3b8' }}>外部サービス（Money Forward）の連携情報：</span>
          ...
          <li className="pl-3">
            <span className="font-medium" style={{ color: '#94a3b8' }}>システム利用ログおよびCookie：</span>
      ...
      <section className="space-y-2">
        <h2 className="font-semibold" style={{ color: '#f1f5f9' }}>2. 利用目的</h2>
      ...
      <section className="space-y-3">
        <h2 className="font-semibold" style={{ color: '#f1f5f9' }}>3. 外部送信およびサードパーティ連携</h2>
        <p>本サービスは、コア機能の提供にあたり、以下のサードパーティにデータを送信・委託しています。</p>
        <div className="overflow-x-auto">
          <table className="w-full text-xs border-collapse" style={{ borderColor: '#1e293b' }}>
            <thead>
              <tr style={{ background: '#0f172a', color: '#94a3b8' }}>
                <th className="text-left p-2.5 border" style={{ borderColor: '#1e293b' }}>送信先</th>
                <th className="text-left p-2.5 border" style={{ borderColor: '#1e293b' }}>送信データ</th>
            ...
            <tbody>
              {[
                ...
                <tr key={i} style={{ background: i % 2 === 0 ? '#0a0a10' : '#0d1117' }}>
                  <td className="p-2.5 border font-medium" style={{ borderColor: '#1e293b', color: '#94a3b8', verticalAlign: 'top', whiteSpace: 'nowrap' }}>{dest}</td>
                  <td className="p-2.5 border" style={{ borderColor: '#1e293b', verticalAlign: 'top' }}>{data}</td>
      ...
      <section className="space-y-2">
        <h2 className="font-semibold" style={{ color: '#f1f5f9' }}>4. データの共有（世帯機能に関する特記事項）</h2>
      ...
      <section className="space-y-2">
        <h2 className="font-semibold" style={{ color: '#f1f5f9' }}>5. 安全管理措置</h2>
        <p>本サービスは、お預かりしたデータの漏洩、滅失、または毀損を防止するため、以下のセキュリティ対策を講じています。</p>
        <ul className="space-y-2 pl-4" style={{ borderLeft: '2px solid #334155' }}>
          <li className="pl-3"><span className="font-medium" style={{ color: '#94a3b8' }}>通信保護：</span> 全てのデータ通信におけるSSL/TLSによる暗号化。</li>
          <li className="pl-3"><span className="font-medium" style={{ color: '#94a3b8' }}>厳格なアクセス制御：</span> Supabaseの行レベルセキュリティ（RLS）を用いた、世帯単位での強固なデータベース層アクセス遮断。</li>
          <li className="pl-3"><span className="font-medium" style={{ color: '#94a3b8' }}>機密情報の隠蔽：</span> 外部連携用認証情報の暗号化保持、およびエラーログ（api_error_logs）における個人特定情報のマスク処理。</li>
        </ul>
      </section>

      <section className="space-y-2">
        <h2 className="font-semibold" style={{ color: '#f1f5f9' }}>6. 個人情報の開示、訂正、利用停止、削除および退会</h2>
      ...
      <section className="space-y-2">
        <h2 className="font-semibold" style={{ color: '#f1f5f9' }}>7. プライバシーポリシーの改定</h2>
