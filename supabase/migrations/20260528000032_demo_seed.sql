-- =============================================================
-- デモユーザー用シードデータ & リセット関数
--
-- 使い方:
--   1. Supabase Dashboard > Authentication でメール認証を有効化
--   2. demo@kai-app.example.com / パスワード を作成
--   3. 作成した user_id をコピーして下の SELECT で確認
--        SELECT id FROM auth.users WHERE email = 'demo@kai-app.example.com';
--   4. SELECT setup_demo_household('<user_id>');
--
-- リセット（毎日 Cron から呼ぶ）:
--   SELECT reset_demo_data('demo@kai-app.example.com');
-- =============================================================
...
BEGIN
  ...
  -- 住居費・日用品カテゴリがなければ作成（旧バージョンからのリセット対応）
...
-- GRANT（service_role から呼べるよう）
GRANT EXECUTE ON FUNCTION public.setup_demo_household(uuid)    TO service_role;
GRANT EXECUTE ON FUNCTION public._seed_demo_transactions(uuid,uuid,uuid,uuid,uuid,uuid,uuid,uuid,uuid,uuid) TO service_role;
GRANT EXECUTE ON FUNCTION public._seed_demo_goals(uuid)        TO service_role;
GRANT EXECUTE ON FUNCTION public._seed_demo_monthly_data(uuid) TO service_role;
