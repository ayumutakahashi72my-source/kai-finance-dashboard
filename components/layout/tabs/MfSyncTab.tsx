              ...
              <div style={{ position: 'fixed', left: '50%', top: '50%', zIndex: 61, transform: 'translate(-50%, -50%)', width: 'min(360px, calc(100vw - 40px))', background: 'rgba(18,16,28,0.98)', border: '1px solid rgba(255,255,255,.14)', borderRadius: 20, padding: '24px 22px 20px', display: 'flex', flexDirection: 'column', gap: 14, boxShadow: '0 24px 64px rgba(0,0,0,0.7)' }}>
                ...
                {!browserMode ? (
                  <>
                    ...
                    <input type="text" inputMode="numeric" maxLength={6} value={otpCode} onChange={e => setOtpCode(e.target.value.replace(/\D/g, ''))} onKeyDown={e => { if (e.key === 'Enter') handleOtpSubmit() }} placeholder="123456" autoFocus style={{ width: '100%', boxSizing: 'border-box', padding: '14px 16px', borderRadius: 12, background: 'rgba(255,255,255,.05)', border: `1px solid ${otpError ? RED + '66' : 'rgba(255,255,255,.14)'}`, color: TEXT1, fontSize: 24, fontWeight: 800, letterSpacing: '0.3em', textAlign: 'center', fontFamily: 'var(--font-mono),monospace', outline: 'none' }}/>
