...
import { CORAL, BLUE, GREEN, RED, AMBER, TEXT1, TEXT2, TEXT3, TEXT4, SyncResult, BackBtn } from './_shared'
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {loading ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, padding: '4px 0' }}>
          ...
          {[1,2,3].map(i => <div key={i} style={{ height: 64, borderRadius: 16, background: 'rgba(255,255,255,.02)', animation: `kai-blink 1.4s ${i * 0.15}s steps(2) infinite` }}/>)}
        </div>
      ) : (
        <>
          {/* Hero card */}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {MF_FEATURES.map((f, i) => (
              <div key={f.title} style={{ display: 'flex', alignItems: 'center', gap: 14, background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.07)', borderRadius: 16, padding: '14px 16px', animation: `kai-rise .5s ${.1 + i * .07}s ease-out both` }}>
          ...
          <div style={{ display: 'flex', gap: 10, paddingBottom: 8, animation: 'kai-rise .5s .42s ease-out both' }}>
            <button type="button" onClick={onBack} style={{ flex: 1, padding: '15px', textAlign: 'center', background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.12)', borderRadius: 99, color: TEXT2, fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>戻る</button>
            <button type="button" onClick={handleSync} disabled={syncing} style={{ flex: 2, padding: '15px', background: enabled ? `linear-gradient(135deg, ${CORAL} 0%, ${BLUE} 100%)` : 'rgba(255,255,255,.06)', border: enabled ? 'none' : '1px solid rgba(255,255,255,.14)', borderRadius: 99, color: enabled ? '#0c0a14' : TEXT3, fontSize: 15, fontWeight: 800, cursor: syncing ? 'not-allowed' : 'pointer', fontFamily: 'inherit', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: enabled ? `0 8px 24px ${CORAL}44` : 'none', opacity: syncing ? 0.7 : 1, transition: 'opacity .2s' }}>
          ...
          {otpPending && (
            <>
              ...
              <div style={{ position: 'fixed', left: '50%', top: '50%', zIndex: 61, transform: 'translate(-50%, -50%)', width: 'min(360px, calc(100vw - 40px))', background: 'rgba(18,16,28,0.98)', border: '1px solid rgba(255,255,255,.14)', borderRadius: 20, padding: '24px 22px 20px', display: 'flex', flexDirection: 'column', gap: 14, boxShadow: '0 24px 64px rgba(0,0,0,0.7)' }}>
                <div style={{ display: 'flex', gap: 6, background: 'rgba(255,255,255,.05)', borderRadius: 10, padding: 4 }}>
                  {(['code', 'browser'] as const).map(mode => (
                    <button key={mode} onClick={() => { setBrowserMode(mode === 'browser'); setOtpError(null) }} style={{ flex: 1, padding: '7px 4px', borderRadius: 7, fontSize: 12, fontWeight: 600, border: 'none', cursor: 'pointer', fontFamily: 'inherit', background: (mode === 'browser') === browserMode ? 'rgba(255,255,255,.12)' : 'transparent', color: (mode === 'browser') === browserMode ? TEXT1 : TEXT3 }}>
                ...
                {!browserMode ? (
                  <>
                    ...
                    <input type="text" inputMode="numeric" maxLength={6} value={otpCode} onChange={e => setOtpCode(e.target.value.replace(/\D/g, ''))} placeholder="123456" autoFocus style={{ width: '100%', boxSizing: 'border-box', padding: '14px 16px', borderRadius: 12, background: 'rgba(255,255,255,.05)', border: `1px solid ${otpError ? RED + '66' : 'rgba(255,255,255,.14)'}`, color: TEXT1, fontSize: 24, fontWeight: 800, letterSpacing: '0.3em', textAlign: 'center', fontFamily: 'var(--font-mono),monospace', outline: 'none' }}/>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button onClick={clearOtp} style={{ flex: 1, padding: '11px', borderRadius: 12, fontSize: 13, fontWeight: 600, background: 'rgba(255,255,255,.05)', border: '1px solid rgba(255,255,255,.10)', color: TEXT2, cursor: 'pointer', fontFamily: 'inherit' }}>キャンセル</button>
                      <button onClick={handleOtpSubmit} disabled={otpCode.length < 6 || syncing} style={{ flex: 2, padding: '11px', borderRadius: 12, fontSize: 13, fontWeight: 700, background: otpCode.length >= 6 && !syncing ? `linear-gradient(135deg,${CORAL} 0%,${BLUE} 100%)` : 'rgba(255,255,255,.06)', border: 'none', color: otpCode.length >= 6 && !syncing ? '#0c0a14' : TEXT3, cursor: otpCode.length >= 6 && !syncing ? 'pointer' : 'not-allowed', fontFamily: 'inherit', opacity: syncing ? 0.7 : 1 }}>{syncing ? '認証中…' : '認証する'}</button>
                    </div>
                  </>
                ) : (
                  <>
                    ...
                    <input type="password" value={sessionCookie} onChange={e => setSessionCookie(e.target.value)} placeholder="_moneyforward_session=... または値のみ" style={{ width: '100%', boxSizing: 'border-box', padding: '11px 14px', borderRadius: 12, background: 'rgba(255,255,255,.05)', border: `1px solid ${otpError ? RED + '66' : 'rgba(255,255,255,.14)'}`, color: TEXT1, fontSize: 11, fontFamily: 'var(--font-mono),monospace', outline: 'none' }}/>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button onClick={clearOtp} style={{ flex: 1, padding: '11px', borderRadius: 12, fontSize: 13, fontWeight: 600, background: 'rgba(255,255,255,.05)', border: '1px solid rgba(255,255,255,.10)', color: TEXT2, cursor: 'pointer', fontFamily: 'inherit' }}>キャンセル</button>
                      <button onClick={handleBrowserSessionSubmit} disabled={!sessionCookie.trim() || syncing} style={{ flex: 2, padding: '11px', borderRadius: 12, fontSize: 13, fontWeight: 700, background: sessionCookie.trim() && !syncing ? `linear-gradient(135deg,${CORAL} 0%,${BLUE} 100%)` : 'rgba(255,255,255,.06)', border: 'none', color: sessionCookie.trim() && !syncing ? '#0c0a14' : TEXT3, cursor: sessionCookie.trim() && !syncing ? 'pointer' : 'not-allowed', fontFamily: 'inherit', opacity: syncing ? 0.7 : 1 }}>{syncing ? '取込中…' : '取込む'}</button>
                    </div>
                  </>
                )}
              </div>
          ...
          {showNoCredsAlert && (
            <>
              ...
              <div style={{ position: 'fixed', left: '50%', top: '50%', zIndex: 61, transform: 'translate(-50%, -50%)', width: 'min(320px, calc(100vw - 40px))', background: 'rgba(18,16,28,0.98)', border: '1px solid rgba(255,255,255,.14)', borderRadius: 20, padding: '28px 24px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, boxShadow: '0 24px 64px rgba(0,0,0,0.7)' }}>
                ...
                <div style={{ display: 'flex', gap: 8, width: '100%', marginTop: 4 }}>
                  <button onClick={() => setShowNoCredsAlert(false)} style={{ flex: 1, padding: '11px', borderRadius: 12, fontSize: 13, fontWeight: 600, background: 'rgba(255,255,255,.05)', border: '1px solid rgba(255,255,255,.10)', color: TEXT2, cursor: 'pointer', fontFamily: 'inherit' }}>閉じる</button>
